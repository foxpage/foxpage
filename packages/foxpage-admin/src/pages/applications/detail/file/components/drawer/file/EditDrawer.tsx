...
const EditDrawer: React.FC<EditDrawerProps> = (props: EditDrawerProps) => {
  ...
  const handleSave = () => {
    ...
    if (editFile?.applicationId) {
      saveFile({ applicationId: editFile.applicationId, folderId: editFile.folderId }, () => {
        // close drawer
        closeDrawer(false);

        // refresh file list
        fetchFileList({
          applicationId: editFile.applicationId || '',
          id: editFile.folderId,
          page: pageInfo.page,
          size: pageInfo.size,
        });
      });
