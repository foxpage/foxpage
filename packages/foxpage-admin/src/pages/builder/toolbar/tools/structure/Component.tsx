...
import { ComponentStructure, DndInfoType } from '@/types/builder';
...
type ContainerProps = ReturnType<typeof mapStateToProps> & typeof mapDispatchToProps & DProps;
...
const Container: React.FC<ContainerProps> = (props) => {
  ...
  const handleChange = (dragInfo: any = {}, desc: any = {}) => {
    ...
    if (dragInfo.method === 'INSERT') {
      ...
      if (targetComponent && targetComponent.wrapper) {
        const container = componentList.find(
          (item: ComponentStructure) => item.id === targetComponent.wrapper,
        );
    ...
    if (desc.type === 'add') {
      const wrapper = newWrapperComponent(registeredComponent, desc.name, finalParentId);
      const params = {
          ...
          if (clientY >= top + 10 && clientY <= bottom - 10) {
            ...
            if (close === 'false') {
              ...
              if (firstChildNode) {
                ...
                const newRectInfo = newRect(
                  rect.top - ROOT_TOP,
                  rect.width,
                  rect.height,
                  rect.bottom - ROOT_TOP,
                );
                Object.assign(
                  newDragInfo,
                  newDnd(
  ...
  return (
    <Drawer
      title={<DrawerHeader>{global.structure}</DrawerHeader>}
      placement="left"
      visible
      destroyOnClose
      mask={false}
      ...
      <div id="structure-container" style={{ position: 'relative', flex: 1 }}>
        <React.Fragment>
          {TreeView}
          <Placeholder />
          <Tools />
