...
import { ComponentStructure, ExtensionData } from '@/types/builder';

import { BLANK_NODE, iconStyle, SYSTEM_PAGE } from '../../../../constant';
...
interface Type {
  ...
  baseStructure: ExtensionData['baseStructureRecord'];
...
const Node: React.FC<Type> = (props) => {
  ...
  const extended = !!component.extension?.extendId || !!baseStructure[component.id];
  const cannotMove = component.name === BLANK_NODE || extended;
  ...
  return visible ? (
    <Component
      ...
      <LayerContent>
        <Content>
          <Name state={component.name === BLANK_NODE}>{component.label}</Name>
        </Content>
      </LayerContent>
      <TooBar>
        ...
        {component.id !== SYSTEM_PAGE && (
          <DragIcon
            ...
            title={cannotMove ? 'Cannot move it' : 'Move'}
            cannotmove={cannotMove ? 'true' : 'false'}
