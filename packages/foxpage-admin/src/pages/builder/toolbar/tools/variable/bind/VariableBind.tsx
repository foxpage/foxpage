...
import { updateMock } from '@/actions/builder/more';
...
const getFinalProps = (selectedComponent: ComponentStructure, keys: string, mock?: boolean) => {
  const componentProps = !mock
    ? _.cloneDeep(selectedComponent.props) || {}
    : _.cloneDeep(selectedComponent.mock) || {};
  const keyPath: string[] = keys.split('.') || [];
  const key = keyPath.pop() as string;
  const finalProps = keyPath.reduce((a: string | ComponentPropsType, c: string) => {
...
const VariableBind: React.FC<VariableBindProps> = (props) => {
  ...
  const { open, keys = '', type, mock } = variableBindParams;
  ...
  useEffect(() => {
    if (open) {
      ...
      const { finalProps, key } = getFinalProps(selectedComponent, keys, mock);
  ...
  const handleClose = () => {
    setVariableBindModalVisibleStatus({ open: false, keys: '', type: EditorInputEnum.Text, mock: false });
  };

  const handleSave = () => {
    const { finalProps, key, componentProps } = getFinalProps(selectedComponent, keys, mock);
    if (value === undefined) {
    ...
    updateEditorValue(mock ? 'mock' : 'props', componentProps);
    if (mock) updateMockValue({ applicationId, folderId, value: componentProps });
    save({ applicationId, folderId, isWrapper: false });
    handleClose();
  };
