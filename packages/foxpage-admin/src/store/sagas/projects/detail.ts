...
import { ProjectFileActionType } from '@/reducers/projects/detail';
...
function* save(action: ProjectFileActionType) {
  ...
  const { editFile, pageInfo } = store.getState().projects.detail;
  if (!editFile.type) {
...
function* handleDeleteFile(action: ProjectFileActionType) {
  ...
  const { pageInfo } = store.getState().projects.detail;
  const {
