  ...
  async index(@Ctx() ctx: FoxCtx, @Body() params: UpdateContentReq): Promise<ResData<Content>> {
    try {
      ...
      const result = await this.service.content.info.updateContentDetail(
        Object.assign({}, params, { type: <FileTypes>TYPE.MOCK }),
        { ctx, actionType: [LOG.UPDATE, TYPE.MOCK].join('_') },
      );

      if (result.code === 1) {
        return Response.warning(i18n.mock.invalidMockContentId, 2191201);
      } else if (result.code === 2) {
        return Response.warning(i18n.mock.invalidIdType, 2191202);
      } else if (result.code === 3) {
        return Response.warning(i18n.mock.mockNameExist, 2191203);
      }
      ...
      return Response.error(err, i18n.mock.updateMockContentFailed, 3191201);
    }
  }
}
