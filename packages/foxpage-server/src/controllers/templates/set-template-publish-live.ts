...
// migration to pages/set-page-publish-live.ts
  ...
  async index(@Ctx() ctx: FoxCtx, @Body() params: VersionPublishStatusReq): Promise<ResData<ContentVersion>> {
    try {
      ...
      const result = await this.service.version.live.setVersionPublishStatus(params as VersionPublish, {
        ctx,
        liveRelation: true,
        actionType: [LOG.LIVE, TYPE.TEMPLATE].join('_'),
      });

      if (result.code === 1) {
        return Response.warning(i18n.template.templateVersionHasPublished, 2071201);
      ...
      return Response.error(err, i18n.template.setTemplatePublishStatusFailed, 3071201);
    }
  }
}
