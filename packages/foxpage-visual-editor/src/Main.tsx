    ...
    <EditorContext.Provider value={editorContextValue}>
      <FlexLayout>
        ...
        <Panel style={{ flex: editorExpend ? `0 0 ${editorShrink ? MAX_WIDTH : WIDTH}px` : '0 0 0px' }}>
          <Editor selectNode={selectNode} onShrink={setEditorShrink} onExpend={handleExpend} />
          <MockEditor
            selectNode={(selectNode?.__mock as RenderStructureNode) || null}
            visible={mockerVisible}
            visibleChange={setMockerVisible}
          />
        </Panel>
      </FlexLayout>
